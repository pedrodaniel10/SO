...
void inicializarTarefas() {
	int i;
	ibanco_log = open("./log.txt",O_CREAT|O_WRONLY|O_APPEND,0777);
...
void* efetuarComando(void *idTarefa) {
	while(TRUE) {
		...
		switch (comando.operacao){
			case DEBITAR:
				comando.result = debitar(comando.idConta1, comando.valor);
				break;

			case CREDITAR:
				comando.result = creditar(comando.idConta1, comando.valor);
				break;

			case TRANSFERIR:
				comando.result = transferir(comando.idConta1, comando.idConta2, comando.valor);
				break;

			case LER_SALDO:
				if ((comando.valor = lerSaldo(comando.idConta1)) < 0)
					comando.result = CMD_NOT_OK;
				else
					comando.result = CMD_OK;
				break;
			...
			default:
				comando.result = CMD_ERROR;
		}
		write(comando.ficheiro_i_banco, (void*) &comando, sizeof(comando));
