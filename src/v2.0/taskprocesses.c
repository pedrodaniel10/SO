...
pthread_mutex_t trincoColocar, trincoRetirar, trincosContas[NUM_CONTAS];
...
void* efetuarComando() {
	while(TRUE) {
		...
		switch (comando.operacao){
			...
			case LER_SALDO:
				if ((comando.valor=lerSaldo(comando.idConta)) < 0)
					printf("%s(%d): Erro.\n\n", COMANDO_LER_SALDO, comando.idConta);
				else
					printf("%s(%d): O saldo da conta é %d.\n\n", COMANDO_LER_SALDO, comando.idConta, comando.valor);
				break;
